export default {
  scale: function(incr, frameConfig) {
    this.scalePlane("#artwork", incr);
    if (frameConfig.renderMat) {
      this.setMat(frameConfig,"#artwork");
    }
    this.adjustFrame(frameConfig);
  ...
  setMat: function(frameConfig,artworkId) {
    ...
    var mat_padding = frameConfig.matPad * g.width;
  ...
  adjustFrame: function(frameConfig) {
    if (frameConfig.renderMat) {
      ...
      var FRAME_DEPTH = frameConfig.depth * g.width;
      var FRAME_WIDTH = frameConfig.width * g.width;
  ...
  adjustImageDimensions: function(frameConfig) {
